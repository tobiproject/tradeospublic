...
export function DayOfWeekChart({ data }: Props) {
  const hasData = data.some(d => d.tradeCount > 0)

  return (
    <Card className="border-border/60">
      <CardHeader className="pb-2">
        <CardTitle className="text-base">Ø P&L nach Wochentag</CardTitle>
      </CardHeader>
            ...
            <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="pnl" radius={[3, 3, 0, 0]}>
                {data.map((d, i) => (
                  <Cell
                    key={i}
                    fill={d.tradeCount === 0 ? '#374151' : d.pnl >= 0 ? '#22c55e' : '#ef4444'}
                    fillOpacity={d.tradeCount === 0 ? 0.3 : 0.85}
                  />
                ))}
              </Bar>
            </BarChart>
